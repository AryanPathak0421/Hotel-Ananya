        ...
        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-700 max-w-[1400px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                {statCards.map(stat => {
                    ...
                    return (
                        <div key={stat.label} className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all group relative overflow-hidden active:scale-95 duration-500">
                            ...
                            <div className="flex justify-between items-start mb-6 relative z-10">
                                <div className={`p-4 rounded-2xl ${stat.color} transition-all group-hover:shadow-lg shadow-sm border border-transparent group-hover:border-white/50`}>
                                    <Icon size={24} />
                                </div>
                            ...
                            <div className="relative z-10">
                                <p className="text-4xl font-black text-secondary tracking-tighter tabular-nums mb-2">{stat.value}</p>
                                <div className="space-y-1">
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] leading-none mb-1">{stat.label}</p>
                                    <p className="text-[8px] text-slate-300 font-bold italic tracking-tight">{stat.sub}</p>
                                </div>
                            </div>
                        </div>
                    );
                ...
                <div className="lg:col-span-2 space-y-8">
                    <div className="bg-white border border-slate-100 rounded-[3rem] p-10 shadow-sm overflow-hidden relative">
                        <div className="flex items-center justify-between mb-10 pb-6 border-b border-slate-50">
                            <div>
                                <h3 className="text-2xl font-black text-secondary tracking-tight lowercase">Guest <span className="text-primary italic">manifest.</span></h3>
                                <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.2em] mt-1">Live synchronization in progress</p>
                            </div>
                            <Link to="/admin/bookings" className="bg-slate-50 text-[9px] font-black text-secondary uppercase tracking-[0.2em] py-3 px-6 rounded-2xl hover:bg-secondary hover:text-white transition-all flex items-center gap-2 group shadow-sm">
                        ...
                        <div className="space-y-6">
                            {recentBookings.length > 0 ? recentBookings.map((bk) => (
                                <div key={bk.id || bk._id} className="p-6 rounded-[2.5rem] bg-slate-50/30 border border-slate-100/50 hover:bg-white hover:shadow-2xl hover:shadow-slate-200/50 transition-all duration-500 flex flex-wrap items-center gap-8 group">
                                    <div className="w-16 h-16 bg-secondary text-primary rounded-[1.5rem] flex items-center justify-center text-2xl font-serif font-black shadow-2xl shadow-secondary/20 shrink-0 group-hover:scale-110 transition-transform duration-500 rotate-3 group-hover:rotate-0">
                                        {bk.user?.name?.[0] || '?'}
                                    </div>
                                    <div className="flex-1 min-w-[180px]">
                                        <div className="flex items-center gap-3 mb-1.5">
                                            <h4 className="font-black text-secondary text-sm uppercase tracking-tight">{bk.user?.name}</h4>
                                            <span className={`text-[7px] px-2.5 py-1 rounded-full font-black uppercase tracking-widest border ${bk.bookingStatus === 'confirmed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-amber-50 text-amber-600 border-amber-100'
                                                }`}>
                                                {bk.bookingStatus}
                                            </span>
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest italic">{bk.bookingId}</p>
                                            <p className="text-[9px] text-primary font-black uppercase tracking-tight leading-none truncate max-w-[150px]">{bk.plan?.planName}</p>
                                        </div>
                                    </div>

                                    <div className="flex items-center gap-6 border-x border-slate-100/50 px-8">
                                        <div className="text-center">
                                            <p className="text-[7px] font-black text-slate-300 uppercase tracking-widest mb-1.5 italic">Inbound</p>
                                            <div className="bg-white text-secondary px-3 py-1.5 rounded-xl shadow-sm border border-slate-50">
                                                <p className="text-base font-black tabular-nums">{new Date(bk.checkIn).getDate()}</p>
                                            </div>
                                        </div>
                                        <ChevronRight size={14} className="text-primary/30" />
                                        <div className="text-center">
                                            <p className="text-[7px] font-black text-slate-300 uppercase tracking-widest mb-1.5 italic">Outbound</p>
                                            <div className="bg-white text-secondary px-3 py-1.5 rounded-xl shadow-sm border border-slate-50">
                                                <p className="text-base font-black tabular-nums">{new Date(bk.checkOut).getDate()}</p>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="text-right ml-auto flex flex-col items-end">
                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Valuation</p>
                                        <p className="text-xl font-black text-secondary tracking-tighter tabular-nums leading-none">₹{bk.totalPrice?.toLocaleString()}</p>
                                        <div className="mt-2 flex items-center gap-2">
                                            <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                                            <span className="text-[8px] font-black text-emerald-600 uppercase tracking-widest">Pre-paid</span>
                                        </div>
                                    </div>
                                </div>
                            )) : (
